...
class ConfigManager(private val dataFolder: File) {
    private val configFile = File(dataFolder, "overrides.toml")
     ...
     * If the file doesn't exist, it will be recreated with default values.
     */
    fun reload() {
        if (!configFile.exists()) {
            ChestExporter.instance.logger.info("Configuration file not found, creating a new one.")
            createDefaultConfig()
        }
        config.load()
        ChestExporter.instance.logger.info("Configuration reloaded.")
    }
